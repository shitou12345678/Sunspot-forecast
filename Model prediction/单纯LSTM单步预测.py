...
预测第24太阳周，只用一个LSTM层，单步预测，用前一步预测后一步
"""
# 数据结构处理
def parser(x):
...
train, test = supervised_values[0:-132], supervised_values[-132:]

# 将训练集和测试集都缩放到[0, 1]之间
scaler, train_scaled, test_scaled = scale(train, test)


# 构建一个LSTM网络模型，并训练，样本数：1，循环训练次数：3000，LSTM层神经元个数为8
lstm_model = fit_lstm(train_scaled, 1, 200, 100)
# 重构输入数据的形状，
print(train_scaled)
...
# 求真实值和预测值之间的标准差
rmse = sqrt(mean_squared_error(raw_values[-132:], predictions))
print('Test RMSE: %.3f' % rmse)
# 作图展示
plt.plot(raw_values[-132:], 'r.-')
